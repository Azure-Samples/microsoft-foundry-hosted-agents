...
namespace foundry_agent
{
          ...
          instructions_(std::move(instructions))
    {
        ...
        {
            throw ConfigError{"Agent instructions must not be empty."};
        }
    }

    Config Config::FromEnvironment(std::string instructions)
    {
        ...
        return Config{std::move(endpoint), std::move(model), std::move(instructions)};
    }
    ...
    const std::string &Config::Instructions() const noexcept
    {
